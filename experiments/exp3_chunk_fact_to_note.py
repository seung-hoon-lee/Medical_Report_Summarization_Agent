#!/usr/bin/env python3
"""
Experiment 3 baseline: Chunk-to-Fact-to-Note.
...
Pipeline position in the paper comparison:
    ...
    3. Chunk-to-Fact-to-Note  <- THIS         o                 x                x
    4. Iterative Multi-Agent Fact-to-Note     o                 o                x
    5. Ours                                   o                 o                o

Method definition:
    Stage 2 first CHUNKED the raw record into per-document sections
    (operative report, preoperative outpatient note, discharge record, ...)
    and extracted facts per chunk (Stage2_Mode = agent1_only). A SINGLE
    generation agent receives the merged chunk-level facts from inputs/e2.csv
    column "Extracted_Facts" and writes the outpatient note in one pass. The
    raw "Input" column is intentionally NOT given to the generator: in this
...
METHOD_NAME = "chunk_fact_to_note"
EVIDENCE_TAG = "CURRENT_ROW_FACTS"

DEFAULT_MODEL = "qwen3.6:35b"
DEFAULT_INPUT_CSV = Path("/root/DY/Agents/inputs/e2.csv")
DEFAULT_OUTPUT_CSV = Path("/root/DY/Agents/outputs/exp3_chunk_fact_to_note.csv")
DEFAULT_AUDIT_JSONL = Path("/root/DY/Agents/outputs/exp3_chunk_fact_to_note_audit.jsonl")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Experiment 3 baseline: Chunk-to-Fact-to-Note. A single agent generates the outpatient note "
            "from facts extracted per document chunk. No professor style."
        )
    )
