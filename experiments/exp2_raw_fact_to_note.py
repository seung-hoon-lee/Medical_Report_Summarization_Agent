#!/usr/bin/env python3
"""
Experiment 2 baseline: Raw-to-Fact-to-Note.

Pipeline position in the paper comparison:
    Method                                Fact Extraction  Iterative Agent  Few-shot Style
    1. Raw-to-Note                            x                 x                x
    2. Raw-to-Fact-to-Note    <- THIS         o                 x                x
    3. Chunk-to-Fact-to-Note                  o                 x                x
    4. Iterative Multi-Agent Fact-to-Note     o                 o                x
    5. Ours                                   o                 o                o

Method definition:
    Stage 2 already extracted facts from the WHOLE raw record in a single pass
    (no chunking; Stage2_Mode = raw_input_agent1_only). A SINGLE generation
    agent receives those facts from inputs/e1.csv column "Extracted_Facts"
    and writes the outpatient note in one pass. The raw "Input" column is
    intentionally NOT given to the generator: in this pipeline the fact list
    is the complete patient evidence.
...
METHOD_NAME = "raw_fact_to_note"
EVIDENCE_TAG = "CURRENT_ROW_FACTS"

DEFAULT_MODEL = "qwen3.6:35b"
DEFAULT_INPUT_CSV = Path("/root/DY/Agents/inputs/e1.csv")
DEFAULT_OUTPUT_CSV = Path("/root/DY/Agents/outputs/exp2_raw_fact_to_note.csv")
DEFAULT_AUDIT_JSONL = Path("/root/DY/Agents/outputs/exp2_raw_fact_to_note_audit.jsonl")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Experiment 2 baseline: Raw-to-Fact-to-Note. A single agent generates the outpatient note "
            "from facts extracted from the whole raw record (no chunking). No professor style."
        )
    )
