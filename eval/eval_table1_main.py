#!/usr/bin/env python3
"""
Table 1 — Main comparison: LLM-as-a-Judge evaluation of the 5 pipelines.

    Method                                 Metrics
    1. Raw-to-Note                         Faithful Pass / Hallucination-free /
    2. Raw-to-Fact-to-Note                 Unsupported per Note / Critical Complete /
    3. Chunk-to-Fact-to-Note               Style Win
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Table 1 (main comparison) LLM-as-a-Judge evaluation.")
...
def judge_completeness(client: JudgeClient, gt: str, note: str, args: argparse.Namespace) -> dict[str, Any]:
    ...
    absent_facts = [
        {"fact": clean_scalar(f.get("fact")), "category": clean_scalar(f.get("category"))}
        for f in facts
        if isinstance(f, dict) and clean_scalar(f.get("status")).lower() == "absent"
    ]
    return {
...
def evaluate(args: argparse.Namespace) -> int:
    ...
    def evaluate_record(method: str, record_id: str) -> dict[str, Any]:
        ...
        if not row["note"]:
            # An empty note asserts nothing (0 unsupported) but covers nothing:
            # run completeness against the empty note so n_absent == every GT
            # critical fact. Table 3 derives "missing" from this field.
            comp = judge_completeness(client, row["gt"], "", args)
            result.update(
                likert=1, n_claims=0, n_unsupported=0, unsupported_claims=[],
                hallucination_free=False, faithful_pass=False, faithful_borderline=False,
                n_critical=comp["n_critical"], n_absent=comp["n_absent"], n_partial=comp["n_partial"],
                absent_facts=comp["absent_facts"], complete_strict=False, complete_lenient=False,
                style_score=None, empty_note=True,
            )
        ...
        result.update(
            ...
            absent_facts=comp["absent_facts"],
    ...
    print(f"\n=== Table 1: Main comparison (style opponent: {METHODS[args.style_opponent][0]}) ===")
...
"""
export OPENAI_API_KEY=sk-...
python eval_table1_main.py --judge_model gpt-4o --sample_per_professor 10
python eval_table1_main.py --sample_per_professor 0     # full 1050 records (expensive)
python eval_table1_main.py --dry_run                    # plumbing test, no API
"""
